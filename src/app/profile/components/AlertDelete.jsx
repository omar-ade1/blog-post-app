...
const AlertDelete = ({ setShowConfirm, handleDelete }) => {
  ...
  useEffect(() => {
    if (checkWord.toLowerCase() == "delete my account") {
      setDisable(false);
    } else {
      setDisable(true);
    }
    ...
    <div>
      ...
      <div className="max-w-full min-w-[300px] bg-white dark:bg-gray-800 p-5 font-bold text-center rounded fixed top-1/2 left-1/2 z-20 transform -translate-x-1/2 -translate-y-1/2">
        <h3 className="capitalize text-xl dark:text-white">
          please write (<span className="normal-case font-bold text-red-600 dark:text-red-500">delete my account</span>) in this input
        </h3>
